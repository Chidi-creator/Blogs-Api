...
export default class TagRepository {
    ...
    async updateTag(id: mongoose.Types.ObjectId, updateData: Partial<ITag>): Promise<ITag | null> {
      try {
        const updatedTag = await Tag.findByIdAndUpdate(id, { $set: updateData }, { new: true });
        return updatedTag;
      } catch (error: any) {
